...
def overlay_image_with_polygon(bgr_im, poly: np.ndarray):
    # Reshape points in a form required by polylines
    points = poly.reshape((-1, 1, 2)).astype(np.int32)

    # Draw the polygon
    res = cv2.polylines(bgr_im, [points], isClosed=True, color=(0, 255, 0), thickness=3)

    # Display the result
    cv2.imshow("overlay_image_with_polygon", res)
    cv2.waitKey(1)
